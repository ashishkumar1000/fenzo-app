...
- 2026-09-15 — Second-round review (TrueSheet fix + native footer + conditional hints + abort deadline): 8 patch findings applied (deadline→TIMEOUT classification with DEADLINE_ABORTED stamp, deadline interceptor tests, Sheet dismiss-on-unmount, timer self-cleanup, hint positive-path tests, whitespace-only hint coverage, unused tokens removed, formatting). 2 defers to deferred-work.md. Touched suites 36/36 green; full suite 87/93 (6 pre-existing geolocation-chain failures); tsc at the 78-error pre-existing baseline.
...
**Second-round review (2026-09-15, TrueSheet fix + conditional hints + abort deadline):**

- [x] [Review][Patch] Deadline aborts surface as CANCELLED, not TIMEOUT — abort-filtering callers swallow real timeouts [src/services/api/apiClient.ts:87-103, src/services/api/apiError.ts:77-83] **APPLIED:** the deadline stamps the config with `DEADLINE_ABORTED` before aborting; `toApiError` checks the stamp before the CANCELLED branch and reports `TIMEOUT` — a stalled fetch now shows the error/Retry view, and the sheet shows the timeout copy, deterministically (the stamp exists only on deadline aborts, so caller cancels stay `CANCELLED`).
- [x] [Review][Patch] The abort-deadline interceptor has zero test coverage [src/services/api/apiClient.ts, __tests__/] **APPLIED:** 4 deadline tests in `api-client.test.ts` (fires at API_TIMEOUT → TIMEOUT ApiError, caller abort → CANCELLED, pre-aborted signal → immediate CANCELLED, settled request clears its timer) + 2 classification tests in `api-error.test.ts` (stamped ERR_CANCELED → TIMEOUT, unstamped → CANCELLED), via a signal-honoring fake adapter.
- [x] [Review][Patch] Sheet.tsx has no dismiss-on-unmount — popping the screen while the sheet is presented strands the native sheet (the same failure class the `detail ?` gate fixes, on the one path the gate can't cover: a 401 forced-logout navigation reset or deep-link reset with the sheet open) [src/components/ui/Sheet.tsx] **APPLIED:** unmount cleanup dismisses via the ref while still presented.
- [x] [Review][Patch] A rejection that reaches the response interceptor with `error.config` undefined orphans the 15s timer [src/services/api/apiClient.ts:106-112] **APPLIED:** the timer self-cleans from the WeakMap at fire time (and stamps before aborting), so an orphaned entry can never outlive its fired timer; the residual case is a single harmless no-op abort of a detached controller, documented at `clearRequestTimer`.
- [x] [Review][Patch] Dropped-edit hints only tested on the negative path — no test asserts a hint APPEARS when a field is cleared or the technician is deselected; a regression making hints never show would pass the suite [__tests__/edit-job-sheet.test.tsx] **APPLIED:** positive-path test — clearing the description fires the text hint, deselecting the technician fires the unassign hint, typing real text clears the hint again.
- [x] [Review][Patch] Whitespace-only edits are dropped by `buildPatch` with no hint — hint conditions only cover "saved nonempty → draft empty"; typing spaces into an empty field or whitespace-only changes vs stored text are silently dropped [src/features/jobDetail/components/EditJobSheet.tsx:160-166] **APPLIED:** `textDropped` mirrors `textChanged`'s full drop rule (trimmed empty OR trimmed equal to saved) and fires on the raw draft; copy generalized to "Empty or spacing-only text won't save — the field keeps its saved value." (regression test for the whitespace-only case added).
- [x] [Review][Patch] Unused spacing tokens s24/s28/s40 added (only s32 is consumed) [src/theme/spacing.ts] **APPLIED:** removed; s32 kept with a comment naming its consumer.
- [x] [Review][Patch] Formatting drift in new code — `import {colors, palette, ...}` (no brace spaces) and inline `style={{marginTop: spacing.s1}}` deviate from the surrounding prettier style [src/features/jobDetail/components/EditJobSheet.tsx:30, 240] **APPLIED:** brace spacing normalized; icon margin extracted to a `hintIcon` style.
...
- [x] [Review][Defer] Screen-test closed-sheet assertions are calibrated against the TrueSheet Jest mock's always-rendered tree (device renders nothing when closed) — deferred, inherent mock-fidelity limit; a mock or library change would need these recalibrated [__tests__/job-detail-screen.test.tsx]
- [x] [Review][Defer] Per-request abort listener is never removed from a reused caller AbortController (one closure per request until it aborts) — deferred, latent: current call sites create a fresh controller per request, so nothing accumulates [src/services/api/apiClient.ts:97-99]
